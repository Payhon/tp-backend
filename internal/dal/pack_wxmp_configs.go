...
func UpsertPackWxMpConfig(ctx context.Context, tx *gorm.DB, app *PackWxMpConfig) error {
	...
	return db.WithContext(ctx).Exec(`
		INSERT INTO pack_wxmp_configs (
			...
			status, home_banner_url, login_logo_url, remark, created_at, updated_at
		)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
		...
		DO UPDATE SET
			...
			updated_at = NOW()
	`, app.ID, app.TenantID, app.OrgID, app.AppID, app.WxAppID, app.AppSecret, app.Status, app.HomeBannerURL, app.LoginLogoURL, app.Remark).Error
}
