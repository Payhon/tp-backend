...
func (*EndUser) GetEndUserList(ctx context.Context, req model.EndUserListReq, claims *utils.UserClaims, dealerScopeID string) (*model.EndUserListResp, error) {
	db := global.DB.WithContext(ctx)

	// dealer 过滤：经销商账号强制限定；厂家账号可选 req.DealerID
	effectiveDealerID := ""
	if dealerScopeID != "" {
		effectiveDealerID = dealerScopeID
	} else if req.DealerID != nil && *req.DealerID != "" {
	...
	if effectiveDealerID != "" {
		base = base.Where("dbat.dealer_id = ?", effectiveDealerID)
	}
	...
	if err := base.Select(selectSQL, effectiveDealerID, effectiveDealerID).
		Group("u.id, u.name, u.phone_number, dealer_id, dealer_name").
...
func (*EndUser) GetEndUserDevices(ctx context.Context, req model.EndUserDeviceListReq, claims *utils.UserClaims, dealerScopeID string) (*model.EndUserDeviceListResp, error) {
	db := global.DB.WithContext(ctx)
	...
	if dealerScopeID != "" {
		base = base.Where("dbat.dealer_id = ?", dealerScopeID)
	}
...
// ForceUnbind 强制解绑（管理员/经销商）
func (*EndUser) ForceUnbind(ctx context.Context, req model.EndUserForceUnbindReq, claims *utils.UserClaims, dealerScopeID string) error {
